...
function ResultPage() {
  ...
  // Loading state
  if (isLoadingAttempt || isLoadingTest || isLoadingAttemptScores || isLoadingQuestionCount) {
    return (
    ...
    return (
      <div className="mx-auto max-w-3xl px-4 py-10">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {test.title} · {attempt.student_name}
        </p>
  ...
  const average =
    total > 0
      ? Math.round((safeAttempts.reduce((s, a) => s + Number(a.score), 0) / total) * 100) / 100
      : 0;
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        {attempt.student_name} · finished in {Math.floor(Number(attempt.time_taken_seconds ?? 0) / 60)}m{" "}
